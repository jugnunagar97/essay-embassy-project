...
const AdminQAManager: React.FC = () => {
  ...
  const handleDeleteLegacy = async () => {
    if (!window.confirm('Are you sure you want to delete ALL legacy Q&A entries (without questionNumber or slug)? This cannot be undone.')) return;
    try {
      const snapshot = await getDocs(collection(db, 'qaLibrary'));
      let count = 0;
      for (const docSnap of snapshot.docs) {
        const data = docSnap.data();
        if (!data.questionNumber || !data.slug) {
          await deleteDoc(doc(db, 'qaLibrary', docSnap.id));
          count++;
        }
      }
      ...
      alert('Failed to delete legacy Q&A entries.');
    }
  };
