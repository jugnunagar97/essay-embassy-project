            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              {related.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">No related questions yet</p>
                  </div>
              ) : (
                related.map((q) => (
                  <Link
                    ...
                    to={`/question/${q.questionNumber}/${q.slug}`}
                      className="group block p-3 rounded-xl border border-gray-200 dark:border-gray-600 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-md transition-all duration-200"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-2 py-1 rounded-full text-xs">
                          <span className="w-1.5 h-1.5 bg-primary-500 rounded-full"></span>
                          #{q.questionNumber}
                        </span>
                        <span className="text-xs text-gray-500 dark:text-gray-400">{q.subject || "General"}</span>
                      </div>
                      <div
                        className="line-clamp-2 text-sm font-medium text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors duration-200"
              ...
              <div className="p-4 space-y-4">
                <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400">Paper Type</span>
                  </div>
                  <span className="font-semibold text-gray-900 dark:text-white">{entry.paperType || "General"}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  <div className="flex items-center gap-2">
                    <svg className="w-4 h-4 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <span className="text-xl font-bold text-primary-600 dark:text-primary-400">${entry.price.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-900 dark:text-white">Intermediate</span>
                </div>
            </div>
          </div>
        </aside>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="bg-gradient-to-r from-green-600 to-green-700 p-6 text-white">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-bold">More from {entry.subject || "this category"}</h3>
              </div>
            </div>
            <div className="p-6">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {(related.length ? related : listQa().filter((q)=>q.status==='published' && q.id!==entry.id).slice(0,6)).map((q) => (
                  <Link 
                    key={q.id} 
                    to={`/question/${q.questionNumber}/${q.slug}`} 
                    className="group block p-4 rounded-xl border border-gray-200 dark:border-gray-600 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-lg transition-all duration-200 transform hover:-translate-y-1"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="inline-flex items-center gap-1 bg-primary-100 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 px-2 py-1 rounded-full text-xs font-medium">
                        #{q.questionNumber}
                      </span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">{q.subject || "General"}</span>
                    </div>
                    <div className="line-clamp-2 text-sm font-medium text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors duration-200" dangerouslySetInnerHTML={{ __html: q.title || q.question }} />
              </Link>
            ))}
              </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <h4 className="font-semibold text-gray-900 dark:text-white mb-2">How do unlocks work?</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">You pay once to access the complete solution with detailed explanations and step-by-step guidance.</p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Can I view the solution later?</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">Yes, unlocked solutions stay available in your browser for future reference.</p>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <h4 className="font-semibold text-gray-900 dark:text-white mb-2">What payment methods do you accept?</h4>
                <p className="text-sm text-gray-600 dark:text-gray-400">We accept all major credit cards and secure payment methods for your convenience.</p>
              </div>
            </div>
        </div>
      </div>
