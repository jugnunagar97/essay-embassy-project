...
const targetDir = './src/pages/Services';
...
for (const file of files) {
    ...
    // Replace grid
    const regex = /const (EssayServicesGrid|ServiceTypesGrid|EssayTypesGrid) = \(\) => \{[\s\S]*?^\};/m;
    if (regex.test(content)) {
        content = content.replace(regex, newGrid);
        console.log('Grid replaced in ' + file);
    } else {
        console.log('Grid NOT FOUND in ' + file);
    }

    // Rename usages of old grid names
    content = content.replace(/<ServiceTypesGrid \/>/g, '<EssayServicesGrid />');
    content = content.replace(/<EssayTypesGrid \/>/g, '<EssayServicesGrid />');

    // Ensure the heading exists
    if (!content.includes('ESSAY WRITING SERVICES WE OFFER')) {
        // Replaces only the first usage of <EssayServicesGrid />
        content = content.replace('<EssayServicesGrid />', `
                ...
                <EssayServicesGrid />`);
        console.log('Added Heading to ' + file);
    }
