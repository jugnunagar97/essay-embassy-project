...
function fixFile(filePath, serviceName) {
  ...
  // Check if has the bug pattern: Helmet inside testimonials.map
  const hasBug = /testimonials\.map\([^)]*\)\s*{[^}]*return\s*\(\s*\n\s+<>\s*\n\s+<Helmet>/s.test(content);
  if (!hasBug) {
    // Check if already fixed
    if (/export default function[^}]*return\s*\(\s*<>\s*<Helmet>/s.test(content)) {
      return false;
    }
    return false;
  }

  // 1. Remove Helmet block from inside testimonials.map
  ...
  if (mainReturnMatch) {
    ...
    // 4. Fix closing tags - find last </div>);} pattern
