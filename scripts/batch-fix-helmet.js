...
function fixFile(filePath, serviceName) {
  ...
  if (content.includes('export default function') && content.match(/return\s*\(\s*<>\s*<Helmet>/)) {
    // Check if Helmet is NOT in testimonials.map
    const lines = content.split('\n');
    let inMap = false;
    let foundHelmet = false;
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('testimonials.map')) inMap = true;
      if (inMap && lines[i].includes('<Helmet>')) return false; // Has bug
      if (lines[i].includes('export default function')) inMap = false;
      if (lines[i].includes('<Helmet>') && !inMap) foundHelmet = true;
    }
    if (foundHelmet) return false; // Already fixed
  }

  // Check if has the bug: Helmet inside testimonials.map
  const bugPattern = /testimonials\.map\([^)]*\)\s*{[^}]*return\s*\(\s*\n\s+<>\s*\n\s+<Helmet>/;
  if (!bugPattern.test(content)) return false;

  // 1. Remove Helmet block from inside map - find the exact pattern
  const mapStart = content.indexOf('testimonials.map');
  if (mapStart === -1) return false;
  
  // Find the return statement inside map
  const mapSection = content.substring(mapStart);
  const returnInMap = mapSection.match(/return\s*\(\s*\n\s+<>\s*\n\s+<Helmet>[\s\S]*?<\/Helmet>\s*\n\s+<div/);
  if (returnInMap) {
    const beforeMap = content.substring(0, mapStart);
    const mapContent = content.substring(mapStart);
    const fixedMap = mapContent.replace(/return\s*\(\s*\n\s+<>\s*\n\s+<Helmet>[\s\S]*?<\/Helmet>\s*\n\s+<div/, 'return (\n                  <div');
    content = beforeMap + fixedMap;
  }
  ...
  const mainReturn = content.match(/(export default function[^}]*return\s*\(\s*)(<div[^>]*>)/);
  if (mainReturn && !content.substring(0, mainReturn.index).includes('<Helmet>')) {
    const indent = mainReturn[1].match(/\s*$/)[0];
    ...
    content = content.replace(mainReturn[0], helmetBlock);
